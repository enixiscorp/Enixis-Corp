...
formEl?.addEventListener('submit', async (e) => {
  ...
  showOrderSummary({
    ...
    try {
      // Générer automatiquement une notification Slack avec boutons pour TOUTES les demandes
      await sendInitialRequestNotificationWithButtons({
        name, email, phone,
        serviceLabel: serviceLabel(service),
        basePrice,
        finalPrice: price,
...
// Fonction pour envoyer une notification initiale avec boutons pour TOUTES les demandes de formulaire
...
💰 Montant estimé: ${formatFcfa(orderData.finalPrice)}${promoInfo}
...
${orderData.issue ? `• Souci mentionné: ${orderData.issue.type}` : ''}
...
🔄 En attente de traitement par l'équipe...
  ...
  try {
    // Créer l'URL de la facture avec toutes les données
    let invoiceUrl = `https://enixis-corp.vercel.app/api/invoice?invoice=${invoiceNumber}&name=${encodeURIComponent(orderData.name || '')}&email=${encodeURIComponent(orderData.email || '')}&phone=${encodeURIComponent(orderData.phone || '')}&service=${encodeURIComponent(orderData.serviceLabel || '')}&price=${orderData.finalPrice || 0}&delivery=${orderData.delivery || 'standard'}&payment=En%20attente`;
    ...
    const payload = {
      ...
      attachments: [
        {
          color: '#ff9500', // Orange pour "en attente"
          title: `📋 NOUVELLE DEMANDE - ${invoiceNumber}`,
          text: `Demande client reçue - Actions disponibles ci-dessous`,
          fields: [
            {
              title: 'Client',
            ...
            {
              title: 'Status Actuel',
              value: '⏳ Nouvelle demande - En attente de traitement',
              short: false
            }
          ],
          actions: [
          ...
          footer: `Demande ${invoiceNumber} - Reçue`,
    ...
    console.log('✅ Notification de demande initiale avec boutons envoyée');

  } catch (error) {
    console.error('❌ Erreur envoi notification demande:', error);
    ...
    try {
      const fallbackPayload = {
        ...
        attachments: [{
          color: '#ff9500',
          title: `📋 NOUVELLE DEMANDE - ${invoiceNumber}`,
          text: `Demande client reçue (boutons non disponibles)`,
          fields: [
            {
              title: 'Client',
      ...
      console.log('✅ Notification demande envoyée (sans boutons)');
    } catch (fallbackError) {
      console.error('❌ Erreur fallback notification demande:', fallbackError);
    }
  }
}
